...
def write_to_excel(api_data, output_file, provider, service_name):
    wb = Workbook()
    ws = wb.active
    ws.title = "API Methods"

    service_label = service_name or provider.upper()
    if provider == "aws":
        target_label = f"{service_label} API Action"
    elif provider == "gcp":
        target_label = f"{service_label} REST Resource"
    else:
        target_label = f"{service_label} Resource Operation"
    ...
    if provider == "aws":
        ...
        for item in api_data:
            ...
            ws.append([action_name] + [""] * num_levels)
